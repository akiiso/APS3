...
public class Enemy extends Entity {
	...
	public void tick() {
		if (this.isCollidingWithPlayer() == false) {
			if ((Game.player.getX() + 128 > this.getX())
					&& (Game.player.getX() - 128 < this.getX())) {
	...
	public boolean isCollidingWithPlayer() {
		Rectangle enemyCurrent = new Rectangle(this.getX() + mask_X,
				this.getY() + mask_Y, mask_W, mask_H);

		Rectangle player = new Rectangle(
				Game.player.getX() + Game.player.mask_X,
				Game.player.getY() + Game.player.mask_Y, Game.player.mask_W,
				Game.player.mask_H);
	...
	public boolean isColliding(int xnext, int ynext) {
		Rectangle enemyCurrent = new Rectangle(xnext + mask_X, ynext + mask_Y,
				mask_W, mask_H);
		...
		for (int i = 0; i < Game.enemies.size(); i++) {
			...
			if (e == this)
				continue;
			Rectangle targetEnemy = new Rectangle(e.getX() + mask_X,
					e.getY() + mask_Y, mask_W, mask_H);
	...
	public void render(Graphics g) {
		...
		if (dir == right_dir) {
			g.drawImage(EnemyMoveRight[index], this.getX() - Camera.x,
		...
		{
			g.drawImage(EnemyMoveLeft[index], this.getX() - Camera.x,
					this.getY() - Camera.y, null);
		}
